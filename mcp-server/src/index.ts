  ...
  tools: [
    {
      name: 'get_today',
      description:
        "Get today's complete health summary: calories eaten vs goal, macros, water intake, weight, and workouts. Use this first when asked about today's status.",
    ...
    },
    {
      name: 'log_meal',
      description:
        'Log a meal. First call list_foods to find the exact food name, then call this. The food name must match exactly.',
      inputSchema: {
        type: 'object',
        properties: {
  ...
  try {
    switch (name) {
      ...
      case 'get_today': {
        ...
        const totalCal = Math.round(meals.reduce((s: number, m: any) => s + (m.calories ?? 0), 0));
        const totalProtein = round1(meals.reduce((s: number, m: any) => s + (m.protein_g ?? 0), 0));
        const totalCarbs = round1(meals.reduce((s: number, m: any) => s + (m.carbs_g ?? 0), 0));
        const totalFat = round1(meals.reduce((s: number, m: any) => s + (m.fat_g ?? 0), 0));
        const totalWater = drinks.filter((x: any) => x.type === 'water').reduce((s: number, x: any) => s + x.amount_ml, 0);
        const totalDrinks = drinks.reduce((s: number, x: any) => s + x.amount_ml, 0);
        ...
        const result = {
          ...
          workouts: workouts.map((w: any) => ({ type: w.type, duration_minutes: w.duration_minutes, notes: w.notes })),
          total_workout_minutes: workouts.reduce((s: number, w: any) => s + (w.duration_minutes ?? 0), 0),
        };
      ...
      case 'get_meals': {
        ...
        const meals = data ?? [];
        const totals = {
          calories: Math.round(meals.reduce((s: number, m: any) => s + (m.calories ?? 0), 0)),
          protein_g: round1(meals.reduce((s: number, m: any) => s + (m.protein_g ?? 0), 0)),
          carbs_g: round1(meals.reduce((s: number, m: any) => s + (m.carbs_g ?? 0), 0)),
          fat_g: round1(meals.reduce((s: number, m: any) => s + (m.fat_g ?? 0), 0)),
        };
      ...
      case 'get_weight_history': {
        const limit = (a.limit as number) ?? 14;
        const { data, error } = await supabase
          .from('weight_log')
        ...
        const weights = entries.map((e: any) => e.weight_kg);
      ...
      // ── list_foods ──────────────────────────────────────────────────────────
      case 'list_foods': {
        const search = a.search as string | undefined;
        let query = supabase
          .from('foods')
      ...
      // ── log_meal ────────────────────────────────────────────────────────────
      case 'log_meal': {
        const foodName = a.food_name as string;
        const mealType = a.meal_type as string;
      ...
      case 'log_weight': {
        ...
        const { error } = await supabase.from('weight_log').upsert(
          { user_id: USER_ID, date: d, weight_kg: weight, notes: (a.notes as string) ?? null },
      ...
      case 'log_drink': {
        ...
        const amount = a.amount_ml as number;
        const d = (a.date as string) ?? today();
        ...
        const totalWater = (todayDrinks ?? []).filter((x: any) => x.type === 'water').reduce((s: number, x: any) => s + x.amount_ml, 0);
      ...
      case 'log_workout': {
        ...
        const { data: inserted, error } = await supabase.from('workout_log').insert({
          ...
          calories_burned: (a.calories_burned as number) ?? null,
      ...
      // ── update_goals ────────────────────────────────────────────────────────
      case 'update_goals': {
        const updates: Record<string, unknown> = { user_id: USER_ID, updated_at: new Date().toISOString() };
      ...
      case 'get_weekly_stats': {
        ...
        const dayTotals: Record<string, { cal: number; protein: number }> = {};
        for (const m of meals as any[]) {
          if (!dayTotals[m.date]) dayTotals[m.date] = { cal: 0, protein: 0 };
        ...
        const weightValues = (weights as any[]).map((w) => Number(w.weight_kg));
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              weight: {
                entries: (weights as any[]).map((w) => ({ date: w.date, weight_kg: Number(w.weight_kg) })),
                change_kg: weightChange,
              },
              workouts: {
                total_sessions: workouts.length,
                total_minutes: (workouts as any[]).reduce((s, w) => s + (w.duration_minutes ?? 0), 0),
                types: [...new Set((workouts as any[]).map((w) => w.type))],
              },
            }, null, 2),
          }],
        };
